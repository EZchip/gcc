...
static void arc_cpu_set (void);

/* Which cpu we're compiling for (A5, ARC600, ARC601, ARC700) */
enum processor_type arc_cpu = PROCESSOR_NONE;
...
/* Specify target option handler.  */ 
#undef TARGET_HANDLE_OPTION
#define TARGET_HANDLE_OPTION arc_handle_option

/* Flags for processor version.  */
int TARGET_A4;
int TARGET_A5;
int TARGET_ARC600;
int TARGET_ARC601;
...
arc_secondary_reload (bool in_p, rtx x, enum reg_class cl,
...
/* -mbarrel_shifter option specified.  */
int barrel_shifter;
...
/* FIXME */
  /* If none of the target option (-mA5,-mARC600,-mARC700) is given,
...
/* Identify CPU variant from -mcpu= or -m<arch>.  */
static void 
arc_cpu_set (void)
{
  struct arc_cpu_type *t;

  if (TARGET_A4)
    error ("ARCtangent-A4 is not supported.");

  /* At this point, handle_option has set TARGET_* based on deprecated
     -m<arch> flags.  Issue warning or error messages if desired.  
     These flags will be set below based on -mcpu<arch>.  */
  if (TARGET_A5 || TARGET_ARC600 || TARGET_ARC700)
    {
#if ISSUE_MARCH_WARNING
      warning (0, "-m<arch> options are deprecated.  Use -mcpu= instead.");
#endif
#if ISSUE_MARCH_ERROR
      error ("-m<arch> options are not supported.  Use -mcpu= instead.");
#endif
    }
  ...
  if (arc_cpu_string[0])
    {
      if (TARGET_A5 || TARGET_ARC600 || TARGET_ARC700)
        warning (0, "-m<arch> options are ignored when -mcpu= is specified.");

      for (t = arc_cpu_types; t->string && strcasecmp (t->string, arc_cpu_string); t++) ;
      arc_cpu = t->ptype;
      if (arc_cpu == 0)
	{
	  warning (0, "-mcpu=%s not recognized, assuming -mcpu=ARC700.", arc_cpu_string);
	  arc_cpu = PROCESSOR_ARC700;
  	  arc_cpu_string = arc_cpu_types[PROCESSOR_ARC700].string;
	}
    }
  else 
    {
      /* Translate -m<arch> option into mcpu= option.  */
      if (TARGET_A5)
	arc_cpu = PROCESSOR_A5;
      if (TARGET_ARC600)
	arc_cpu = PROCESSOR_ARC600;
      if (TARGET_ARC700)
	arc_cpu = PROCESSOR_ARC700;
      for (t = arc_cpu_types; t->string && t->ptype != arc_cpu; t++) ;
	arc_cpu_string = t->string;
    }

  if (TARGET_A4 + TARGET_A5 + TARGET_ARC600 + TARGET_ARC700 > 1)
    warning (0, "multiple -m<arch> options specified, -mcpu=%s assumed.", arc_cpu_string);

  switch (arc_cpu) 
    {
    case PROCESSOR_A5:		TARGET_A5 = 1; break;
    case PROCESSOR_ARC600:	TARGET_ARC600 = 1; break;
    case PROCESSOR_ARC601:	TARGET_ARC601 = 1; break;
    case PROCESSOR_NONE:	/* Fall thru to default.  */
    case PROCESSOR_ARC700:	TARGET_ARC700 = 1; break;
    }
...
/*  Process options.  */
static bool
arc_handle_option (size_t code, const char *arg ATTRIBUTE_UNUSED, int value ATTRIBUTE_UNUSED)
{
  switch (code)
    {
    case OPT_mA4:
  ...
  if (sibthunk_insn)
    {
      ...
      XVECEXP (sibthunk_insn, 0, 0) = gen_rtx_RETURN (VOIDmode);
  ...
  if (GET_CODE (addr) == LABEL_REF)
      ...
      if (GET_CODE (addr) == PLUS)
	{
	  ...
	  if ((GET_CODE (op0) == LABEL_REF
		...
		pat = plus_constant (base, INTVAL (pat));
	      else
		{
		    ...
		    int ignore ATTRIBUTE_UNUSED)
{
  ...
  switch (fcode)
	...
	{
	    /* Accept up to 32-byte (not BIT) alignment.  */
	    int align = get_pointer_alignment (arg0, 32 * BITS_PER_UNIT);
	    if (align)
	    {
